...
export default function BusinessSectors({ settings }: BusinessSectorsProps) {
  ...
  const handleCardClick = (sector: BusinessSector) => {
    ...
    // Smooth scroll to the top of the business sectors section to view the expanded details box
    const sectionEl = document.getElementById('business-sectors-section');
    if (sectionEl) {
  ...
  return (
    <section id="business-sectors-section" className="space-y-10 scroll-mt-20">
      {/* Section Header */}
      ...
      {/* Details Box - Expanded directly above the grid */}
      <AnimatePresence mode="wait">
        {selectedSector && (
          ...
          >
            <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-[35px] border border-slate-200/50 dark:border-white/10 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.4)] dark:shadow-[inset_0_1.5px_0_rgba(255,255,255,0.15)] rounded-[2.5rem] p-6 md:p-10 relative text-left text-gray-900 dark:text-white w-full max-w-7xl mx-auto">
              {/* Close Button */}
              <button
                onClick={closeModal}
                className="absolute top-6 right-6 p-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-550 dark:text-slate-350 rounded-xl transition-all active:scale-95 cursor-pointer border-none"
              >
                <X className="w-4 h-4" />
              </button>

              {/* Header */}
              <div className="flex items-center gap-4 border-b border-gray-150/40 dark:border-slate-800 pb-5 mb-8 pr-8">
                ...
                <div>
                  ...
                  <span className="text-[10px] font-bold text-blue-650 dark:text-blue-400 uppercase tracking-widest mt-0.5 block">
                    {t('รายละเอียดกลุ่มธุรกิจเป้าหมาย', 'Target Industry Details')}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* คืออะไร (What is it?) */}
                <div className="bg-white/50 dark:bg-slate-950/20 border border-gray-150/40 dark:border-slate-800/60 rounded-2xl p-5 md:p-6 relative overflow-hidden flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs md:text-sm text-gray-600 dark:text-slate-300 leading-relaxed font-sans">
                ...
                <div className="bg-white/50 dark:bg-slate-950/20 border border-gray-150/40 dark:border-slate-800/60 rounded-2xl p-5 md:p-6 relative overflow-hidden flex flex-col justify-between">
                  <div>
                    <h4 className="font-display font-bold text-sm text-gray-900 dark:text-white flex items-center gap-2 mb-4">
