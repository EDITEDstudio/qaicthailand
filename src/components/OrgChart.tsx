  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-3">
        <h2 className="text-2xl md:text-3xl font-bold font-display tracking-tight text-gray-900">
      ...
      <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center max-w-4xl mx-auto">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="block w-full pl-10 pr-4 py-2 text-xs bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none placeholder-gray-400 font-sans"
          />
          {searchTerm && (
            <button
      ...
      <div className="p-4 overflow-x-auto rounded-3xl bg-white border border-gray-100/60 shadow-inner flex flex-col justify-center items-center min-h-[400px]">
        {rootNodes.length === 0 ? (
      ...
      {selectedMember && (
        <div className="fixed inset-0 bg-black/65 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl relative border border-gray-100 flex flex-col md:flex-row animate-in zoom-in-95 duration-200">
            ...
            <div className="w-full md:w-2/5 bg-gray-50 p-6 border-b md:border-b-0 md:border-r border-gray-100 relative flex flex-col items-center justify-center gap-4">
              <button
                ...
                className="absolute top-4 left-4 p-1.5 rounded-full bg-white text-gray-500 hover:text-gray-800 border border-gray-100 md:hidden cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 font-sans mt-2">{selectedMember.email}</p>
                <p className="text-[11px] text-gray-400 font-sans">{selectedMember.phone}</p>
              </div>
              ...
              <div className="w-full space-y-1.5 pt-3 border-t border-gray-200/60">
                ...
                <div className="space-y-1">
                  {selectedMember.certifications.map((cert) => (
                    <div key={cert} className="flex items-center gap-1.5 text-xs text-gray-600 font-sans">
            ...
            <div className="w-full md:w-3/5 p-6 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button
                    onClick={() => setSelectedMember(null)}
                    className="p-1.5 rounded-full bg-gray-50 text-gray-400 hover:text-gray-800 border border-gray-100 hidden md:block cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <p className="text-[10px] text-gray-400 font-sans border-t border-gray-100 pt-3 text-center mt-3">
                {lang === 'TH'
...
function EmployeeCardNode({ node, lang, mini = false, onClick }: EmployeeCardNodeProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white border rounded-2xl cursor-pointer p-4 transition-all duration-150 flex items-center gap-3 w-72 h-24 ${highlightClass}`}
    >
      ...
      <div className="overflow-hidden space-y-0.5">
        ...
        <h4 className="text-xs font-bold text-gray-900 leading-tight line-clamp-1">
        ...
        {node.certifications.length > 0 && (
          <div className="flex gap-1 overflow-hidden pt-1">
            {node.certifications.slice(0, 2).map((cert, idx) => (
              <span
                key={idx}
                className="inline-block bg-gray-50 text-gray-500 text-[8px] px-1 py-0.5 rounded border border-gray-100 line-clamp-1 max-w-[90px]"
              >
                {cert}
              </span>
            ))}
