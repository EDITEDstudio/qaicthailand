              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('ชื่อบริษัท', 'Company Name')}</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('ประเภทธุรกิจ', 'Business Type')}</label>
                    <input 
                      type="text" 
                      ...
                      className="w-full px-4 py-4 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('จำนวนพนักงาน', 'Number of Employees')}</label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('จำนวน Site/สาขา', 'Number of Sites')}</label>
                    <div className="relative">
            ...
            {step === 2 && (
              ...
              >
                <div className="space-y-4">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-widest block">{t('มาตรฐานที่ต้องการรับรอง', 'Standard Required')}</label>
              ...
              >
                <div className="space-y-6">
                  <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-bold text-gray-900 mb-1">{t('เคยมีระบบจัดการมาตรฐานมาก่อนไหม?', 'Existing Management System?')}</h4>
                        <p className="text-xs text-gray-500">{t('ช่วยเราประเมินระยะเวลาในการตรวจประเมินเบื้องต้น', 'Helps us estimate initial audit duration')}</p>
                      </div>
                      <button 
                      ...
                      >
                        <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${data.hasExistingSystem ? 'left-7' : 'left-1'}`} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-bold text-gray-900 mb-1">{t('ต้องการอบรมมาตรฐานเพิ่มเติมไหม?', 'Need Standards Training?')}</h4>
                        <p className="text-xs text-gray-500">{t('หลักสูตรอบรมเพื่อให้พนักงานเข้าใจข้อกำหนด', 'Training for staff to understand requirements')}</p>
                      </div>
                      <button 
                      ...
                      >
                        <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${data.needsTraining ? 'left-7' : 'left-1'}`} />
                      </button>
                    </div>
                  </div>
                </div>
        ...
        >
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden">
            ...
            <div className="p-8 md:p-12 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('สรุปรายละเอียด', 'Summary Details')}</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between border-b border-gray-50 pb-3">
                      <span className="text-gray-500 text-sm">{t('บริษัท', 'Company')}</span>
                      <span className="font-bold text-gray-900">{data.companyName}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-3">
                      <span className="text-gray-500 text-sm">{t('มาตรฐาน', 'Standard')}</span>
                      <span className="font-bold text-blue-600">{data.standard}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-3">
                      <span className="text-gray-500 text-sm">{t('พนักงาน', 'Employees')}</span>
                      <span className="font-bold text-gray-900">{data.employees}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-3">
                      <span className="text-gray-500 text-sm">{t('สถานที่', 'Sites')}</span>
                      <span className="font-bold text-gray-900">{data.sites}</span>
                    </div>
                  </div>
                </div>

                <div className="bg-gray-50 rounded-3xl p-8 space-y-6 border border-gray-100">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('ประมาณการค่าใช้จ่าย', 'Estimated Investment')}</h4>
                  <div className="space-y-4">
                     <div className="flex justify-between items-end">
                        <span className="text-sm text-gray-500">{t('ใบเสนอราคาเลขที่', 'Quote No.')}</span>
                        <span className="text-xs font-mono font-bold text-gray-400">#QAIC-2025-{(Math.random() * 10000).toFixed(0)}</span>
                     </div>
                     <div className="py-6 border-y border-gray-200">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('งบประมาณรวมโดยประมาณ (เริ่มต้น)', 'Total Estimated Budget (Starting)')}</p>
                        <p className="text-4xl font-display font-bold text-gray-900">
                          ฿{(data.sites * 35000 * (data.employees > 50 ? 1.5 : 1)).toLocaleString()}
                          <span className="text-sm font-sans font-normal text-gray-400 ml-2">/ {t('ปี', 'Year')}</span>
                        </p>
                     </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-8">
                ...
                <button 
                  ...
                  className="px-8 py-5 bg-white border border-gray-200 text-gray-600 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-gray-50 transition-all"
                >
              ...
              <div className="flex justify-center">
                 <button 
                  ...
                  className="text-xs font-bold text-gray-400 hover:text-blue-600 transition-colors flex items-center gap-2"
                 >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="p-6 bg-white rounded-3xl border border-gray-100 flex items-start gap-4">
                ...
                <div>
                   <h5 className="font-bold text-gray-900 text-sm mb-1">{t('ใบรับรองได้รับการรับรองพหุภาคี', 'Multi-Lateral Recognition')}</h5>
                   <p className="text-[10px] text-gray-500 leading-relaxed">IAF / UKAS / NAC Accredited certificate recognized worldwide.</p>
                </div>
             </div>
             <div className="p-6 bg-white rounded-3xl border border-gray-100 flex items-start gap-4">
                ...
                <div>
                   <h5 className="font-bold text-gray-900 text-sm mb-1">{t('บริการเป็นเลิศ', 'Service Excellence')}</h5>
                   <p className="text-[10px] text-gray-500 leading-relaxed">Professional auditors with deep industry knowledge.</p>
                </div>
             </div>
             <div className="p-6 bg-white rounded-3xl border border-gray-100 flex items-start gap-4">
                ...
                <div>
                   <h5 className="font-bold text-gray-900 text-sm mb-1">{t('ระบบตรวจสอบออนไลน์', 'Digital Verification')}</h5>
                   <p className="text-[10px] text-gray-500 leading-relaxed">All certificates can be instantly verified via QR Code.</p>
                </div>
             </div>
          </div>
        </motion.div>
